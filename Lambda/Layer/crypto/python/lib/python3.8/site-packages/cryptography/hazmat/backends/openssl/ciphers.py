...
class _CipherContext(object):
    ...
    def __init__(self, backend, cipher, mode, operation):
        ...
        try:
            ...
            raise UnsupportedAlgorithm(
                f"cipher {cipher.name} in {mode.name if mode else mode} mode is not supported by this backend.",
        ...
        if evp_cipher == self._backend._ffi.NULL:
            ...
            msg += f"is not supported by this backend (Your version of OpenSSL may be too old. Current version: {self._backend.openssl_version_text()}.)"
    ...
    def update_into(self, data, buf):
        if len(buf) < (len(data) + self._block_size_bytes - 1):
            raise ValueError(
                f"buffer must be at least {len(data) + self._block_size_bytes - 1} bytes for this payload"
            )
    ...
    def finalize_with_tag(self, tag):
        if len(tag) < self._mode._min_tag_length:
            raise ValueError(
                f"Authentication tag must be {self._mode._min_tag_length} bytes or longer."
            )
