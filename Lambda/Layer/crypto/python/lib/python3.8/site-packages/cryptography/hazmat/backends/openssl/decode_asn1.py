...
def _decode_general_name(backend, gn):
    if gn.type == backend._lib.GEN_DNS:
        ...
        raise x509.UnsupportedGeneralNameType(
            f"{x509._GENERAL_NAMES.get(gn.type, gn.type)} is not a supported type",
            gn.type,
        )
...
class _X509ExtensionParser(object):
    ...
    def parse(self, backend, x509_obj):
        ...
        for i in range(self.ext_count(backend, x509_obj)):
            ...
            if oid in seen_oids:
                raise x509.DuplicateExtension(f"Duplicate {oid} extension found", oid)
            ...
            try:
                ...
                if ext_data == backend._ffi.NULL:
                    ...
                    raise ValueError(f"The {oid} extension is invalid and can't be parsed")
...
def _asn1_string_to_utf8(backend, asn1_string):
    ...
    if res == -1:
        raise ValueError(f"Unsupported ASN1 string type. Type: {asn1_string.type}")
